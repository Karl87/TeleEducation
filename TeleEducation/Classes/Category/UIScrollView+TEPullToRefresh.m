//
//  UIScrollView+TEPullToRefresh.m
//  TeleEducation
//
...
static CGFloat const TEPullToRefreshViewHeight = 60;

@interface TEPullToRefreshArrow : UIView

@property (nonatomic, strong) UIColor *arrowColor;

@end

@interface TEPullToRefreshView ()

@property (nonatomic, copy) void (^pullToRefreshActionHandler)(void);

@property (nonatomic, strong) TEPullToRefreshArrow *arrow;
@property (nonatomic, strong) UIActivityIndicatorView *activityIndicatorView;
@property (nonatomic, strong, readwrite) UILabel *titleLabel;
@property (nonatomic, strong, readwrite) UILabel *subtitleLabel;
@property (nonatomic, readwrite) TEPullToRefreshState state;
@property (nonatomic, readwrite) TEPullToRefreshPosition position;
...
@end
#pragma mark - UIScrollView (TEPullToRefresh)
#import <objc/runtime.h>
static char UIScrollViewPullToRefreshView;

@implementation UIScrollView (TEPullToRefresh)
...
- (void)addPullToRefreshWithActionHandler:(void (^)(void))actionHandler position:(TEPullToRefreshPosition)position {
    ...
    if(!self.pullToRefreshView) {
        CGFloat yOrigin;
        switch (position) {
            case TEPullToRefreshPositionTop:
                yOrigin = -TEPullToRefreshViewHeight;
                break;
            case TEPullToRefreshPositionBottom:
                yOrigin = self.contentSize.height;
                break;
            default:
                return;
        }
        TEPullToRefreshView *view = [[TEPullToRefreshView alloc] initWithFrame:CGRectMake(0, yOrigin, self.bounds.size.width, TEPullToRefreshViewHeight)];
...
- (void)addPullToRefreshWithActionHandler:(void (^)(void))actionHandler {
    [self addPullToRefreshWithActionHandler:actionHandler position:TEPullToRefreshPositionTop];
}

- (void)triggerPullToRefresh {
    self.pullToRefreshView.state = TEPullToRefreshStateTriggered;
    [self.pullToRefreshView startAnimating];
}

- (void)setPullToRefreshView:(TEPullToRefreshView *)pullToRefreshView {
...
- (TEPullToRefreshView *)pullToRefreshView {
...
- (void)setShowsPullToRefresh:(BOOL)showsPullToRefresh {
    ...
    if(!showsPullToRefresh) {
        ...
        if (!self.pullToRefreshView.isObserving) {
            ...
            self.pullToRefreshView.frame = CGRectMake(0, yOrigin, self.bounds.size.width, TEPullToRefreshViewHeight);
        }
    }
}
...
#pragma mark - TEPullToRefresh
@implementation TEPullToRefreshView
...
- (id)initWithFrame:(CGRect)frame {
    if(self = [super initWithFrame:frame]) {
        ...
        self.state = TEPullToRefreshStateStopped;
...
- (void)layoutSubviews {
    ...
    if(hasCustomView) {
        ...
        switch (self.state) {
            case TEPullToRefreshStateAll:
            case TEPullToRefreshStateStopped:
                self.arrow.alpha = 1;
            ...
            case TEPullToRefreshStateTriggered:
                switch (self.position) {
                    case TEPullToRefreshPositionTop:
                        [self rotateArrow:(float)M_PI hide:NO];
                        break;
            ...
            case TEPullToRefreshStateLoading:
...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary *)change context:(void *)context {
    if([keyPath isEqualToString:@"contentOffset"])
        ...
        self.frame = CGRectMake(0, yOrigin, self.bounds.size.width, TEPullToRefreshViewHeight);
    }
...
- (void)scrollViewDidScroll:(CGPoint)contentOffset {
    if(self.state != TEPullToRefreshStateLoading) {
        ...
        if(!self.scrollView.isDragging && self.state == TEPullToRefreshStateTriggered)
            self.state = TEPullToRefreshStateLoading;
        else if(contentOffset.y < scrollOffsetThreshold && self.scrollView.isDragging && self.state == TEPullToRefreshStateStopped && self.position == TEPullToRefreshPositionTop)
            self.state = TEPullToRefreshStateTriggered;
        else if(contentOffset.y >= scrollOffsetThreshold && self.state != TEPullToRefreshStateStopped && self.position == TEPullToRefreshPositionTop)
            self.state = TEPullToRefreshStateStopped;
        else if(contentOffset.y > scrollOffsetThreshold && self.scrollView.isDragging && self.state == TEPullToRefreshStateStopped && self.position == TEPullToRefreshPositionBottom)
            self.state = TEPullToRefreshStateTriggered;
        else if(contentOffset.y <= scrollOffsetThreshold && self.state != TEPullToRefreshStateStopped && self.position == TEPullToRefreshPositionBottom)
            self.state = TEPullToRefreshStateStopped;
    } else {
        CGFloat offset;
...
- (TEPullToRefreshArrow *)arrow {
    if(!_arrow) {
        _arrow = [[TEPullToRefreshArrow alloc]initWithFrame:CGRectMake(0, self.bounds.size.height-54, 22, 48)];
...
- (void)setTitle:(NSString *)title forState:(TEPullToRefreshState)state {
    ...
    if(state == TEPullToRefreshStateAll)
...
- (void)setState:(TEPullToRefreshState)newState {
    
    if(_state == newState)
        return;
    
    TEPullToRefreshState previousState = _state;
    _state = newState;
    ...
    switch (newState) {
        ...
        case TEPullToRefreshStateLoading:
            ...
            if(previousState == TEPullToRefreshStateTriggered && pullToRefreshActionHandler)
...
#pragma mark - TEPullToRefreshArrow

@implementation TEPullToRefreshArrow
